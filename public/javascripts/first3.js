...
.click();
  ...
  for(var i = 0; i < finall.length-3; i++){
    ...
    var arr4 = finall[i].par4.split(',');
    // console.log(parseFloat(arr[0]));

    var gm1 = Math.cbrt(eval(arr[0])*eval(arr2[0])*eval(arr3[0])*eval(arr4[0]));
    var gm2 = Math.cbrt(eval(arr[1])*eval(arr2[1])*eval(arr3[1])*eval(arr4[1]));
    var gm3 = Math.cbrt(eval(arr[2])*eval(arr2[2])*eval(arr3[2])*eval(arr4[2]));
